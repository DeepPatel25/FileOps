...
const encodePipeline = (source: Buffer | string, format: ImageFormat, quality: number) => {
  ...
  if (format === 'avif') return pipeline.avif({ quality: Math.min(quality, 80), effort: 5 })
...
export const processBatchImages: RequestHandler = async (request, response, next) => {
  ...
  let streaming = false
  const cleanupInputs = () => Promise.all((files ?? []).map((file) => file.path ? rm(file.path, { force: true }).catch(() => undefined) : undefined))
  ...
  try {
    ...
    if (operation === 'convert' && !outputFormats.has(requestedFormat)) {
      await cleanupInputs()
      response.status(400).json({ success: false, error: { message: 'Output format must be PNG, JPEG, WebP, or AVIF' } })
      return
    }

    const usesDisk = files.some((file) => Boolean(file.path))
    const archive = usesDisk ? undefined : new JSZip()
    outputDirectory = usesDisk ? await mkdtemp(join(tmpdir(), 'fileflow-batch-output-')) : undefined
    ...
    for (const [index, file] of files.entries()) {
      ...
      if (metadata.format !== 'png' && metadata.format !== 'jpeg' && metadata.format !== 'webp' && metadata.format !== 'heif') {
        await cleanupAll()
        response.status(400).json({ success: false, error: { message: `${file.originalname} is not a supported image` } })
        return
      }
      const format = operation === 'convert' ? requestedFormat as ImageFormat : metadata.format === 'heif' ? 'avif' : metadata.format
      ...
      if (outputDirectory) {
        ...
        await encodePipeline(source, format, requestedQuality).toFile(outputPath)
        outputSize += (await stat(outputPath)).size
        outputFiles.push({ path: outputPath, name: outputName })
      } else {
        const output = await encodePipeline(source, format, requestedQuality).toBuffer()
    ...
    if (usesDisk && outputDirectory) {
      ...
      await cleanupInputs()
      response.set({ 'Content-Type': 'application/zip', 'Content-Disposition': 'attachment; filename="fileflow-batch-images.zip"', 'X-Processed-Count': String(files.length), 'X-Original-Size': String(originalSize), 'X-Compressed-Size': String(outputSize), 'X-Reduction-Percent': String(Math.round((1 - outputSize / originalSize) * 100)) })
      const stream = new ZipArchive({ store: true })
      const cleanup = () => void rm(completedOutputDirectory, { recursive: true, force: true })
      response.on('close', cleanup); stream.on('error', (error: Error) => response.destroy(error)); stream.pipe(response)
      streaming = true
      outputFiles.forEach((file) => stream.file(file.path, { name: file.name })); await stream.finalize(); return
    }
