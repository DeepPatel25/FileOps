...
const loadPdf = async (buffer: Buffer) => {
  try {
    ...
    if (error instanceof Error && /encrypted/i.test(error.message)) {
      throw new Error(
        "Password-protected PDFs are not supported by the page organizer",
      );
    }
    throw error;
  }
...
export const previewPdfPages: RequestHandler = async (
  request,
  response,
  next,
) => {
  if (!request.file) {
    response
      .status(400)
      .json({ success: false, error: { message: "Please upload a PDF file" } });
    return;
  }
  ...
  try {
    ...
    if (pageCount < 1 || pageCount > MAX_PAGES) {
      response
        ...
        .json({
          success: false,
          error: {
            message: `PDF organizer supports between 1 and ${MAX_PAGES} pages`,
          },
        });
      return;
    }
    ...
    await runFile(
      ...
      [
        ...
        "240",
        "-jpegopt",
        "quality=65,progressive=y,optimize=y",
        sourcePath,
        prefix,
      ],
      {
    ...
    if (files.length !== pageCount)
      throw new Error("Could not create every page preview");
    ...
    if (
      ...
      error.message.startsWith("Password-protected")
    ) {
      response
        .status(422)
...
export const organizePdfPages: RequestHandler = async (
  request,
  response,
  next,
) => {
  ...
  try {
    ...
    if (
      ...
      response
        ...
        .json({
          success: false,
          error: {
            message: `Page operations must contain between 1 and ${MAX_PAGES} output pages`,
          },
        });
      return;
    }
    ...
    if (!valid) {
      response
        ...
        .json({
          ...
          error: {
            message:
              "Each page must exist and use a rotation of 0, 90, 180, or 270 degrees",
          },
        });
      return;
    }
    ...
    const copied = await output.copyPages(
      source,
      parsed.map((item) => item.page - 1),
    );
    copied.forEach((page, index) => {
      const operation = parsed[index];
      if (!operation) return;
      page.setRotation(
        degrees((page.getRotation().angle + operation.rotation) % 360),
      );
      output.addPage(page);
    });
