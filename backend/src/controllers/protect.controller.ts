...
const readBoolean = (value: unknown, fallback: boolean) => value === undefined ? fallback : value === 'true'
...
export const protectPdf: RequestHandler = async (request, response, next) => {
  try {
    ...
    if (password.length < 6 || password.length > 128) {
      response.status(400).json({ success: false, error: { message: 'Password must contain between 6 and 128 characters' } })
      return
    }

    const encrypted = await encryptPDF(new Uint8Array(request.file.buffer), password, {
      algorithm: 'AES-256',
