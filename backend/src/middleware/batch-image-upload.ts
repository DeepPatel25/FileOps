...
export const batchImageUploadDirectory = join(tmpdir(), 'fileflow-batch-images')
...
export const batchImageUpload = multer({
  ...
  fileFilter: (_request, file, callback) => {
    if (!supportedTypes.has(file.mimetype)) { callback(new multer.MulterError('LIMIT_UNEXPECTED_FILE', 'files')); return }
    callback(null, true)
  },
})
