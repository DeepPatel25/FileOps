...
const matches = (buffer: Buffer, bytes: number[], offset = 0) =>
...
const allowedFamilies = (file: Uploaded) => {
  ...
  if (["image/avif", "image/heic", "image/heif"].includes(file.mimetype))
    return new Set(["iso-media"]);
  if (file.mimetype.startsWith("video/"))
    return new Set(["iso-media", "matroska", "avi"]);
  if (file.mimetype.startsWith("audio/"))
    return new Set(["iso-media", "mp3", "wav", "ogg", "flac"]);
  return new Set<string>();
};

export const validateFileSignatures: RequestHandler = async (
  request,
  response,
  next,
) => {
    ...
    for (const file of files) {
      ...
      if (!allowedFamilies(file).has(family)) {
        ...
        response.status(400).json({
          success: false,
          error: {
            message: `${file.originalname} content does not match its declared file type`,
          },
        });
        return;
      }
