...
const directoryPrefixes = [
  "fileflow-batch-output-",
  "fileflow-images-",
  "fileflow-media-convert-",
  "fileflow-ocr-",
  "fileflow-pdf-compress-",
  "fileflow-pdf-images-",
  "fileflow-pdf-preview-",
  "fileflow-video-compress-",
  "fileflow-video-split-",
];
const uploadDirectories = [
  "fileflow-batch-images",
  "fileflow-media-uploads",
  "fileflow-pdf-compress-uploads",
  "fileflow-video-uploads",
];
...
export const cleanupStaleTemporaryFiles = async (
  ...
  for (const entry of entries) {
    ...
    if (
      entry.isDirectory() &&
      directoryPrefixes.some((prefix) => entry.name.startsWith(prefix))
    ) {
    ...
    if (entry.isDirectory() && uploadDirectories.includes(entry.name)) {
      ...
      for (const upload of uploads) {
        ...
        if ((await stat(uploadPath)).mtimeMs < cutoff) {
          await rm(uploadPath, {
            recursive: upload.isDirectory(),
            force: true,
          });
          removed++;
        }
  ...
  const run = () =>
    void cleanupStaleTemporaryFiles()
      .then((removed) => {
        if (removed)
          console.log(
            JSON.stringify({ level: "info", event: "temp_cleanup", removed }),
          );
      })
      .catch((error: unknown) =>
        console.error(
